	...
	local autoSellDelay = 30 -- seconds

	-- 1. Ссылка на папку Net
	local netFolder = ReplicatedStorage:WaitForChild("Packages")
		:WaitForChild("_Index")
		:WaitForChild("sleitnick_net@0.2.0")
		:WaitForChild("net")

	-- 2. Ссылки на RemoteFunctions
	local rfSellAll = netFolder:FindFirstChild("RF/SellAllItems")
	local rfSellItem = netFolder:FindFirstChild("RF/SellItem")
	local rfSacrifice = netFolder:FindFirstChild("RF/SacrificeAtlantisSellAll")
	local rfUpdateThreshold = netFolder:FindFirstChild("RF/UpdateAutoSellThreshold")

	-- Проверка доступности в консоль
	if rfSellAll then print("[AutoSell] Найдено: RF/SellAllItems") end
	if rfSacrifice then print("[AutoSell] Найдено: RF/SacrificeAtlantisSellAll") end

	-- Функция Auto Sell
	local function autosell()
		print("[AutoSell] Цикл запущен.")
		...
		while autoSellEnabled do
			...
			for _, item in ipairs(backpack) do
				-- Собираем всё, что не является удочкой
				if item:IsA("Tool") and item.Name ~= "FishingRod" then
					table.insert(itemsToSell, item)
				end
			end

			-- Если в рюкзаке что-то есть (кроме удочки)
			if #itemsToSell > 0 then
				print("[AutoSell] Продаю " .. #itemsToSell .. " предметов...")
				
				-- Попытка 1: Обычная продажа всего
				local success, err = pcall(function()
					return rfSellAll:InvokeServer()
				end)

				-- Попытка 2: Если первая не сработала, пробуем Atlantis Sell
				if not success or err == nil then
					pcall(function()
						rfSacrifice:InvokeServer()
					end)
				end
				
				if success then
					print("[AutoSell] Успешно продано!")
					showNotification("Auto Sell", "Successfully sold " .. #itemsToSell .. " items!", 3)
				else
					warn("[AutoSell] Ошибка при вызове: " .. tostring(err))
				end
			end

			-- Задержка из слайдера
			task.wait(autoSellDelay)
		end
		
		print("[AutoSell] Цикл остановлен.")
	end

	-- Функция для запуска Auto Sell
	local function startAutoSell()
		-- Останавливаем предыдущий цикл если есть
		autoSellEnabled = false
		task.wait(0.1) -- Даём время предыдущему циклу завершиться
		
		-- Запускаем новый цикл
		autoSellEnabled = true
		task.spawn(autosell)

		print("[AutoSell] Запущен с задержкой: " .. autoSellDelay .. " секунд")
		showNotification("Auto Sell", "Started! Selling every " .. autoSellDelay .. " seconds", 3)
	end

	-- Функция для остановки Auto Sell
	local function stopAutoSell()
		autoSellEnabled = false
		print("[AutoSell] Остановлен")
		showNotification("Auto Sell", "Stopped!", 3)
	end
	...
	local function createCategoryFrame(categoryName)
		...
		if categoryName == "Farm" then
			...
			-- Auto Sell Delay slider (5 - 300 seconds)
			createSliderElement(frame, "Auto Sell Delay", 5, 300, 30, function(value)
				autoSellDelay = value
				print("[AutoSell] Задержка изменена на: " .. value .. " секунд")
			end)
		...
		elseif categoryName == "𖦹 Teleport" then
			...
			local locations = {
				...
				{["name"] = "Sewers", ["pos"] = Vector3.new(-1414.0003662109375, -1041.5887451171875, -10411)},
